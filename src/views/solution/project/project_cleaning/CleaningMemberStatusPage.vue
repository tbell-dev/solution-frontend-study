      ...
      <main id="main">
        <section>
          <article class="align1 button-filter">
            <button
              @click="isCleaningOnOff"
              :class="{
                'btn-set btn2-1': isCleaningOn,
                'btn-set btn1-1': !isCleaningOn,
              }"
            >
              정제
            </button>
            ...
            <article>
              <ul class="search-filter">
                ...
                <li class="search-contents">
                  ...
                  />
                  <button class="btn2-1" @click="SortedCleaningData()">
                    검색
                  </button>
                </li>
              </ul>
          ...
          <section>
            <article>
              <ul class="current-bar">
                <li>
                  <button class="btn2-2" @click="cleaningExcelDownload()">
                    <span>엑셀 다운로드</span>
                  </button>
                </li>
              </ul>
            ...
            <article class="basic-article">
              <table class="table1">
                ...
                <tr
                  v-for="(item, index) in CleaningMemberListItem"
                  :key="index"
                >
  ...
  methods: {
    ...
    SortedCleaningData() {
      if (this.searchKeyword) {
        this.searchedData1 = this.CleaningMemberListItemOrigin.filter(data => {
          return data.email
            .toLowerCase()
            .includes(this.searchKeyword.toLowerCase());
        });
        this.searchedData2 = this.CleaningMemberListItemOrigin.filter(data => {
          return data.name
            .toLowerCase()
            .includes(this.searchKeyword.toLowerCase());
        });
        ...
        return (this.CleaningMemberListItem = newArr);
      } else {
        //데이터가 없을 경우, 전체 리스트 출력
        return (this.CleaningMemberListItem =
          this.CleaningMemberListItemOrigin);
      }
    },
    ...
    cleaningExcelDownload() {
      var XLSX = require('xlsx');
      for (const excel of this.CleaningMemberListItem) {
        const excelLogs = {
          No: '',
          이름: '',
          이메일: '',
        ...
        excelLogs.정제 = excel.cleaningCount;
      ...
      XLSX.utils.book_append_sheet(wb, dataWS, '멤버작업현황(정제)'); // workbook에 워크시트 추가, 시트명은 'peopleData'

      XLSX.writeFile(
        wb,
        'tbell_멤버 작업현황_정제_' + year + '.' + month + '.' + date + '.xlsx',
